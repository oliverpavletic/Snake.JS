...
class SnakeGame {
  ...
  spawnFood() {
    ...
    for (var i = 0; i < NUM_FOOD_PIECES - numExistingFoodPieces; i++) {
      ...
      do {
        randomCell = {
          x: Math.floor(Math.random() * GAME_DIMS.WIDTH_IN_CELLS),
          y: Math.floor(Math.random() * GAME_DIMS.HEIGHT_IN_CELLS)
        };
      } while (
        this.foodCoordinates
          .concat(this.snakeCoordinates)
          .some(e => e.x === randomCell.x && e.y === randomCell.y)
      );
  ...
  spawnSnake() {
    ...
    let center = {
      x: Math.floor(GAME_DIMS.WIDTH_IN_CELLS / 2),
      y: Math.floor(GAME_DIMS.HEIGHT_IN_CELLS / 2)
    };
  ...
  addEventListeners() {
    document
      ...
      .addEventListener("click", this.togglePauseScreen.bind(this));
  ...
  nextFrame() {
    ...
    const lastCoordinates = this.snakeCoordinates[
      this.snakeCoordinates.length - 1
    ];
    let nextCoordinates = firstCoordinates;

    this.evalSnakeDirection();

    nextCoordinates = this.getNextCoordinates(
      firstCoordinates,
      this.snakeDirection
    );
  ...
  fatalCollision(nextCoordinates) {
    if (
      nextCoordinates.x > GAME_DIMS.WIDTH_IN_CELLS - 1 ||
      nextCoordinates.y > GAME_DIMS.HEIGHT_IN_CELLS - 1 ||
      nextCoordinates.y < 0 ||
      nextCoordinates.x < 0 ||
      this.snakeCoordinates.some(
        e => e.x === nextCoordinates.x && e.y === nextCoordinates.y
      )
    ) {
      return true;
    }
  ...
  gameOver() {
    ...
    setTimeout(
      () => (document.getElementById("game-over-text").style.zIndex = 3),
      1000
    );
    setTimeout(
      () => (document.getElementById("play-again-text").style.zIndex = 3),
      1000
    );
    document
      .getElementById("play-again-text")
  ...
  removeGameOverDisplay() {
    document
      .getElementById("play-again-text")
      .removeEventListener("click", this.newGame.bind(this));
  ...
  getFoodCollisionIndex(nextCoordinates) {
    return this.foodCoordinates.findIndex(
      e => e.x === nextCoordinates.x && e.y === nextCoordinates.y
    );
  }
...
function getScreenDimensions() {
  ...
  let screenAspectRatio =
    screenDimensions.screenWidth / screenDimensions.screenHeight;
  ...
  if (gameAspectRation < screenAspectRatio) {
    // total height (cells)
    gameHeightInCells =
      GAME_DIMS.HEIGHT_IN_CELLS + 2 * GAME_DIMS.MARGIN_IN_CELLS;
    // cell size (px)
    screenDimensions.cellSize = Math.floor(
      screenDimensions.screenHeight / gameHeightInCells
    );
    // width correction (px)
    screenDimensions.widthCorrection =
      screenDimensions.screenWidth -
      GAME_DIMS.WIDTH_IN_CELLS * screenDimensions.cellSize;
    // top
    screenDimensions.topMargin =
      GAME_DIMS.MARGIN_IN_CELLS * screenDimensions.cellSize;
    ``;
    // side
    ...
    // cell size (px)
    screenDimensions.cellSize = Math.floor(
      screenDimensions.screenWidth / gameWidthInCells
    );
    // height correction
    screenDimensions.heightCorrection =
      screenDimensions.screenHeight -
      GAME_DIMS.HEIGHT_IN_CELLS * screenDimensions.cellSize;
    // top
    screenDimensions.topMargin = screenDimensions.heightCorrection / 2;
    // side
    screenDimensions.sideMargin =
      GAME_DIMS.MARGIN_IN_CELLS * screenDimensions.cellSize;
  }
...
function setupScreen(screenDimensions) {
  ...
  gameOverText.style.top = `${cellSize *
    (GAME_DIMS.HEIGHT_IN_CELLS * 0.5 - 4)}px`;
